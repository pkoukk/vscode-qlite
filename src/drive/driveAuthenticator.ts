...
export class DriveAuthenticator {
  ...
  checkCredentialsConfigured(): Promise<void> {
    return new Promise((resolve, reject) => {
      this.credentialsManager
        .retrievePassword(CREDENTIALS_JSON_SERVICE)
        .then(() => resolve())
  ...
  private showMissingCredentialsMessage(): void {
    ...
    window
      .showWarningMessage(
        `The operation cannot proceed since Google Drive API credentials haven't been configured. Please configure the credentials and try again.`,
        configureButton
      )
      .then((selectedButton) => {
  ...
  private showAddressAlreadyInUse(): void {
    ...
    window
      .showWarningMessage(
        'Oops! It appears that the address ' +
          OAUTH_ENDPOINT +
          ' is already in use! You can set a different port with google.drive.authPort.',
        configurePortButton
      )
      .then((selectedButton) => {
  ...
  private async getAccessToken(): Promise<void> {
    return new Promise(async (resolve, reject) => {
      ...
      window.showInformationMessage(
        'Authorize this app by visiting the external URL'
      );
        ...
        try {
          ...
          if (req.url.indexOf('/?code') > -1) {
            ...
            this.oAuth2Client.getToken(authToken, (err: any, token: any) => {
              ...
              this.credentialsManager
                .storePassword(stringified, TOKENS_JSON_SERVICE)
                .then(() => {
                  // Finally, update the credentials with the generated token
                  this.oAuth2Client.setCredentials(this.token);
                  window.showInformationMessage(
                    'Authorization completed! Now you can access your drive files through VSCode.'
                  );
                  resolve();
                })
                .catch((err) => {
          ...
          if (req.url.indexOf('/?error') > -1) {
            // We do not need the server anymore, so destroy it
            res.end(
              'Authorization flow canceled by user. Please return to VSCode.'
            );
            server.destroy();
      ...
      server.on('error', (err: any) => {
        if (err.toString().includes(PORT_IN_USE_MESSAGE)) {
          ...
          window.showErrorMessage(
            'Unexpected problem while starting HTTP server ' +
              OAUTH_ENDPOINT +
              ': ' +
              err
          );
